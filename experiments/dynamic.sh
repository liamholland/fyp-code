...
for p in 0.2 0.5 0.8 
do
    ./colouring -n $NUMBER_OF_NODES -p $p -A $NUMBER_OF_AUTO_RUNS -S res/dyn_t_ -M $MAXIMUM_ITERATIONS -k m -d t -c 20 -C 20
done
...
for p in 0.2 0.5 0.8 
do
    ./colouring -n $NUMBER_OF_NODES -p $p -A $NUMBER_OF_AUTO_RUNS -S res/dyn_a_ -M $MAXIMUM_ITERATIONS -k m -d a -c 20 -C 20
done
